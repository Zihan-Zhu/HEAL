...
const ChatComponent = ({ conversation, sendMessage, input, setInput, mode }) => {
    ...
    <div>
      <div className="bg-gray-100 p-4 rounded-lg mb-4 h-[36rem] overflow-y-auto">
        {conversation.map((message, index) => (
          <div key={index} className={`mb-2 ${message.role === 'user' ? 'text-black' : 'text-black'}`}>
      ...
      <div className="flex gap-2">
        ...
        <button
          ...
          className={`px-4 py-2 text-white rounded hover:bg-opacity-80 transition-colors duration-300 ${
            mode === 'registration' ? 'bg-blue-500 hover:bg-blue-600' : 'bg-green-500 hover:bg-green-600'
          }`}
        >
          Send
        </button>
...
const App = () => {
  ...
  useEffect(() => {
    // Initial greeting
    const initialMessage = mode === 'monitor'
      ? { role: 'assistant', content: "Welcome to the ED patient monitoring system! Please enter your patient ID to start tracking your condition." }
      : { role: 'assistant', content: "Hello! I'm your emergency department assistant. How can I help you today?" };
  ...
  const sendMessage = async () => {
    ...
    try {
      ...
      if (mode === 'monitor' && response.data.reset_conversation) {
        ...
        setConversation([...conversation, newMessage, { role: 'assistant', content: response.data.response }]);
      }
      ...
      setConversation([...conversation, newMessage, { role: 'assistant', content: "I'm sorry, there was an error processing your request." }]);
    }
  };
  ...
  return (
    <div className="container mx-auto p-4">
      <h1 className="text-3xl font-bold mb-4">Emergency Department Assistant</h1>
      <div className="mb-4">
        <select
          value={mode}
      ...
      {mode === 'dashboard' ? (
        ...
        <>
          ...
          />
          <Notification 
            message={mode === 'monitor' ? "Patient status updated" : "Patient information collected"}
